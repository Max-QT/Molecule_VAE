...
import sys
sys.path.append("D:/Code/chemical_vae-main/Chemical_VAE-1/chemvae")
# print(sys.path)
import os
...
def load_encoder(params):
    ...
    if os.path.exists(params['encoder_weights_file']):
        return load_model(params['encoder_weights_file'])
    else:
        dir = params['encoder_weights_file'].split('encoder.h5')[0]
        files = os.listdir(dir)
        file = [a for a in files if 'encoder' in a][0]
        return load_model(dir+file)
...
def load_decoder(params):
    if os.path.exists(params['decoder_weights_file']):
        if params['do_tgru']:
        ...
        dir = params['decoder_weights_file'].split('decoder.h5')[0]
        files = os.listdir(dir)
        file = [a for a in files if 'decoder' in a][0]
        if params['do_tgru']:
            return load_model(dir+file, custom_objects={'TerminalGRU': TerminalGRU})
        else:
            return load_model(dir+file)
...
def variational_layers(z_mean, enc, kl_loss_var, params):
    ...
    def sampling(args):
        ...
        return K.in_train_phase(z_rand, z_mean)  # # 训练阶段加入高斯噪声，测试阶段去掉，训练时输出z_rand，非训练时输出z_mean
...
def load_property_predictor(params):
    if os.path.exists(params['prop_pred_weights_file']):
        return load_model(params['prop_pred_weights_file'])
    else:
        dir = params['prop_pred_weights_file'].split('prop_pred.h5')[0]
        files = os.listdir(dir)
        file = [a for a in files if 'prop_pred' in a][0]
        return load_model(dir+file)
